    ...
    <div>
      <div id="resear">
        <div class="situation row contact_div">
          <label class="times_contracts">发行商:</label>
        ...
        <div class="situation row contact_div" v-if="editIdss.main_contract_id">
          <label class="times_contracts">操作日志</label>
          <div class="col-sm-10">
            <operate-log :keyLabel="operateLabel" :page="39" :dataId="editIdss.main_contract_id" :rand="rand"></operate-log>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    props: ['editIdss', 'oneditId', 'rand', 'readonly'],
    data() {
      return {
        editId: 0,
        form: {},
    ...
    methods: {
      ...
      getChannelSelectList: function () {
        let url = ENV.HOST_URL + '/companytype/?limit=1000'
        this.$http.get(url)
          .then((res) => {
      ...
      getgamenamesSelectList: function () {
        let url = '/mainContract/gameName?company_id=' + this.editIdss.company_id + '&type=1'
        this.$http.get(url)
          .then((res) => {
