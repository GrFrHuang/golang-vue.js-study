<template>
  <div class="tab-pane active" id="tab_1">
    <section class="content-header">
      <h1>
        渠道合同
        <small>查看渠道合同情况</small>
      </h1>
      <ol class="breadcrumb">
        <li><i class="fa fa-dashboard"></i>渠道回款流</li>
        <li>
          <router-link to="/home/channelContract/channelContractA">合同管理</router-link>
        </li>
      </ol>
    </section>
    <div class="content">
            ...
            <div class="col-lg-10 form-horizontal">
              <div class="form-group">
                <div class="col-lg-6 col-sm-12">
                  ...
                  <label class="col-sm-2 control-label">渠道选择:</label>
                  <div class="col-sm-10 ipt_jianju">
                ...
                <div class="col-lg-6 col-sm-12">
                  ...
                  <div class="col-sm-10 ipt_jianju">
                    <el-select v-model="selectedBusiness" :clearable="true"
                               @change="doGetOrderList" filterable placeholder="请选择">
                      <el-option
            ...
            <div class="box-body">
              ...
              <el-table
                ...
                <el-table-column
                  ...
                  min-width="140">
                  <template scope="scope">
                    <p class="max_widths">{{ scope.row.channel?scope.row.channel.name:'' }}</p>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="company_name"
                  ...
                  min-width="140">
                  <template scope="scope">
                    <p class="max_widths">{{ scope.row.channel_company_name?scope.row.channel_company_name:'' }}</p>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="game_name"
                ...
                <el-table-column
                  ...
                  <template scope="scope">
                    <p class="max_widths">
                      <span v-if="scope.row.begin_time>0">{{ scope.row.begin_time | stampToTime }} ~ {{ scope.row.end_time | stampToTime }}</span>
                    </p>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                  ...
                  <template scope="scope">
                    <p>
                      <span v-if="scope.row.business!=undefined">{{ scope.row.business}}</span>
                    </p>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                  ...
                  <template scope="scope">
                    <p>
                      <span v-if="scope.row.update_user!=undefined">{{ scope.row.update_user.nickname}} {{ scope.row.update_time | stampToTime }}</span>
                    </p>
                  </template>
                </el-table-column>
                <el-table-column
                  ...
                  <template scope="scope">
                    <div class="result_report">
                      ...
                      <el-button title="编辑" class="fa fa-edit" type="text" size="small" @click=goaddContract(scope.row.id)></el-button>
    ...
    <el-dialog
      :title="readonly?'查看渠道合同':'编辑渠道合同'"
      v-model="isShowEditForm"
      top="5%"
      size="small"
      :close-on-click-modal="false">
      <edit :rand="rand" :editId="editId" :readonly="readonly" :flag="flag" :isShowEditForm="isShowEditForm" @close="onEditClose"></edit>
    </el-dialog>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      getBusinessPeople(){
//        var url = '/user/getBusinessPeople/'
...
//            this.ladders.push({username:ladder[i].split(":")[0],password:ladder[i].split(":")[1]})
      ...
      goaddContract: function (id) {
        this.readonly = false
        this.editId = id
        this.isShowEditForm = true

        this.rand = Math.random()
//        this.$store.commit('SET_SHAREDATA', item)
//        this.$router.push({
//          path: '/home/editChannelContract'
//        })
      },
      getGameSelectList: function () {
        var url = '/game/select' + '?flag=qdht' //渠道合同
        this.$http.get(url)
          .then((res) => {
            this.games = res.body
          })
      ...
      getCompanySelectList: function () {
        let url = '/channelCompany/companyname/'
        this.$http.get(url)
          .then((res) => {
      ...
      doGetOrderList: function (page = 0) {
        ...
        if (this.selectedChannels.length != 0) {
          for (let i in this.selectedChannels) {
            channels.push(this.selectedChannels[i])
          }
        }

        if(this.selectedCompanies){
          for (let i in this.selectedCompanies) {
//            for (let j in this.selectedCompanies[i]){
              channels.push(this.selectedCompanies[i])
//            }
          }
        }

//        let status = []
//        if (this.selectContractStatus != null) {
//          status.push(this.selectContractStatus)
//        }

        if(this.selectedBusiness){
          gameIds.push(this.selectedBusiness.gameids)
          channels.push(this.selectedBusiness.channels)
        }
        ...
        let query = util.buildGetParams({
          ...
          "business": this.selectedBusiness?this.selectedBusiness:'',
          "channelcodes": channels.join(","),
//          "flag": "channel",
...
//        let url = '/contract/?' + query
        let url = '/contract/channelContract/?' + query

        let xtoken = sessionStorage.getItem("token")
        this.downloadUrl = ENV.HOST_URL + '/contract/channelDownload?' + query + "&_token=" + xtoken
